...
These instruments raster a high-energy X-ray beam across a specimen, detecting X-ray fluorescence from the material. The resulting data is a spectral cube containing elemental fingerprints from the material across the map. This data can be used to determine the composition at each pixel. Analysis is highly manual and time-consuming, and there is a need for rapid, automated visualisation and class-averaging to inform the user's decision-making during the experiment.
...
- performs dimensionality-reduction (via PCA and UMAP)
